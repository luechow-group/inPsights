...
void Bonds3D::createBonds(AtomsVector3D *atomsVector3D) {
    for (long i = 0; i < atomsVector3D->numberOfEntities(); ++i) {
        for (long j = 0; j < atomsVector3D->numberOfEntities(); ++j) {
            auto distance = Metrics::distance(
                    atomsVector3D->operator[](i).position(),
                    atomsVector3D->operator[](j).position())
                            - (Elements::ElementInfo::vdwRadius(atomsVector3D->operator[](i).type())
                               + Elements::ElementInfo::vdwRadius(atomsVector3D->operator[](j).type())) / 10.0;

            if (distance < bondDrawingLimit_)
