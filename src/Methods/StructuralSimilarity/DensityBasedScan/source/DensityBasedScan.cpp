...
namespace Clustering {
    ...
    DensityBasedScan::DensityBasedScan(const std::shared_ptr<Dataset>& dset)
            : dset_(dset),
              fit_time_(.0),
              predict_time_(.0) {}

    std::shared_ptr<VantagePointTree<Eigen::VectorXf, Clustering::DensityBasedScan::dist >> DensityBasedScan::get_vp() const {
        return vp_tree_;
    }

    void DensityBasedScan::fit() {
        const auto &d = dset_->data();

        const double start = omp_get_wtime();

        vp_tree_ = std::make_shared<VantagePointTree< Eigen::VectorXf, dist >>();
    ...
    const std::vector<double> DensityBasedScan::predictEps(size_t k) {
        const auto &d = dset_->data();
...
#pragma omp parallel for
        for (size_t i = 0; i < d.size(); ++i) {
            std::vector<std::pair<size_t, float>> nlist;
    ...
    uint32_t DensityBasedScan::predict(double eps, size_t min_elems) {
...
#pragma omp ordered
    ...
    void DensityBasedScan::reset() {
        vp_tree_.reset();
        labels_.clear();
    }

    const std::vector<int32_t>& DensityBasedScan::getLabels() const {
        return labels_;
    }

    const double DensityBasedScan::getFitTime() const {
        return fit_time_;
    }

    const double DensityBasedScan::getPredictTime() const {
        return predict_time_;
    }

    void DensityBasedScan::find_neighbors( const std::vector<Eigen::VectorXf>& d, double eps, uint32_t pid,
                                           std::vector<std::pair<size_t, float>>& neighbors ) {
        neighbors.clear();
        vp_tree_->searchByDistance(d[pid], eps, neighbors);
    }

    void DensityBasedScan::prepareLabels(size_t s) {
        labels_.resize(s);
