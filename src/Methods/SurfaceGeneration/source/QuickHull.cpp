...
namespace quickhull {
	...
	HalfEdgeMesh<FloatType, IndexType> QuickHull<FloatType>::getConvexHullAsMesh(const FloatType* vertexData, size_t vertexCount, bool CCW, FloatType epsilon) {
		...
		return HalfEdgeMesh<FloatType, IndexType>(mesh_, vertexData_);
	}
	...
	void QuickHull<T>::createConvexHalfEdgeMesh() {
		...
		while (!faceList.empty()) {
			...
			const vec3& activePoint = vertexData_[tf.mostDistantPoint_];
			...
			while (possiblyVisibleFaces.size()) {
				...
				const auto halfEdges = mesh_.getHalfEdgeIndicesOfFace(mesh_.faces_[mesh_.halfEdges_[faceData.enteredFromHalfEdge_].face_]);
				const std::int8_t ind = (halfEdges[0]==faceData.enteredFromHalfEdge_) ? 0 : (halfEdges[1]==faceData.enteredFromHalfEdge_ ? 1 : 2);
				mesh_.faces_[mesh_.halfEdges_[faceData.enteredFromHalfEdge_].face_].horizonEdgesOnCurrentIteration_ |= (1<<ind);
			}
			...
			if (!reorderHorizonEdges(horizonEdges)) {
				...
				auto it = std::find(tf.pointsOnPositiveSide_->begin(),tf.pointsOnPositiveSide_->end(),activePointIndex);
			...
			for (size_t i = 0; i < horizonEdgeCount; i++) {
				...
				const Eigen::Matrix<T,3,1> planeNormal = mathutils::getTriangleNormal(vertexData_[A],vertexData_[B],activePoint);
	...
	std::array<IndexType,6> QuickHull<T>::getExtremeValues() {
		...
		T extremeVals[6] = {
		        vertexData_[0].x(),vertexData_[0].x(),vertexData_[0].y(),vertexData_[0].y(),vertexData_[0].z(),vertexData_[0].z()};
	...
	bool QuickHull<T>::reorderHorizonEdges(std::vector<IndexType>& horizonEdges) {
		...
		assert(mesh_.halfEdges_[ horizonEdges[horizonEdges.size()-1] ].endVertex_ == mesh_.halfEdges_[ mesh_.halfEdges_[horizonEdges[0]].opp_ ].endVertex_);
		return true;
	}
	...
	MeshBuilder<T> QuickHull<T>::getInitialTetrahedron() {
		...
		if (vertexCount <= 4) {
			...
			const Eigen::Matrix<T,3,1> N = mathutils::getTriangleNormal(vertexData_[v[0]],vertexData_[v[1]],vertexData_[v[2]]);
		...
		const Ray<T> r(vertexData_[selectedPoints.first], (vertexData_[selectedPoints.second] - vertexData_[selectedPoints.first]));
		...
		if (maxD == epsilonSquared_) {
			...
			auto it = std::find_if(vertexData_.begin(),vertexData_.end(),[&](const vec3& ve) {
				return ve != vertexData_[selectedPoints.first] && ve != vertexData_[selectedPoints.second];
			});
			const IndexType thirdPoint = (it == vertexData_.end()) ? selectedPoints.first : std::distance(vertexData_.begin(),it);
			it = std::find_if(vertexData_.begin(),vertexData_.end(),[&](const vec3& ve) {
				return ve != vertexData_[selectedPoints.first] && ve != vertexData_[selectedPoints.second] && ve != vertexData_[thirdPoint];
			});
			const IndexType fourthPoint = (it == vertexData_.end()) ? selectedPoints.first : std::distance(vertexData_.begin(),it);
		...
		const Eigen::Matrix<T,3,1> baseTriangleVertices[]={ vertexData_[baseTriangle[0]], vertexData_[baseTriangle[1]],  vertexData_[baseTriangle[2]] };
		...
		if (maxD == epsilon_) {
			...
			const vec3 extraPoint = N + vertexData_[0];
		...
		for (auto& f : mesh.faces_) {
			...
			const vec3 & va = vertexData_[v[0]];
			const vec3 & vb = vertexData_[v[1]];
			const vec3 & vc = vertexData_[v[2]];
