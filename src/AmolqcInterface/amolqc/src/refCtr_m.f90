...
module refCtrModule
   ...
   use refBaseModule
   use refUtilsModule, only: calcRefDifference, calcRefDiffPerm
   use sortingModule, only: int_sort,quickSortIndex
...
!!! sollte eigentlich refContainerStr als Basisklasse haben. Teste  !!
   ...
   type, extends(referenceContainer) :: refContainerCtr      ! like Struct but electrons are permuted for max likeness and averaged
      ...
      real*8                :: simThreshold = 0.1d0          ! identify structures as similar if max dist is smaller
      real*8                :: sameThreshold = 0.01d0           ! sub list thresh, i.e. for identical structures (w/ spin)
...
contains
   ...
   subroutine refcc_create(this,listLength,doSortFreq,el,refFile,simThreshold,sameThreshold,exclFile)
      ...
      type(reference) :: r,r0
      logical found
      real*8 nucThresh,coreThresh,bondDiff
   ...
   real*8 function refcc_getFirstF(this)
      class(refContainerCtr), intent(inout) :: this
      type(reference), pointer                 :: rp
      type(refl_vlist), pointer                :: p
   ...
   subroutine refcc_insert(this,r)
      ...
      type(refl_vlist)                         :: rl
      type(refl_vlist), pointer                :: p
      real*8                                   :: x(ne,3),xx(3),rr(3)
      real*8                                   :: f,f0,wgt,dist,d,thr,maxdist,meandist,smallestDist
   ...
   subroutine refcc_calcDistances(this,r,mode,maxIdx,outIdx,meanDist,maxDist,idxList)
      ...
      real*8 smallestDist,smallestMean,dmean,dmax
      type(reference), pointer                 :: rp
      type(refl_vlist), pointer                :: p
   ...
   subroutine refcc_writeFullList(this,fname,str)
      ...
      integer, allocatable                  :: idx(:)
      real*8 xx(ne,3),sg(ne,3)
      integer, parameter :: iu1 = 12
      integer, parameter :: iu2 = 13
      real*8, parameter  :: thresh = 0.002d0
      ...
      do i=1,getNNuc()
         write(iu1,'(i4,x,a2,x,3f14.7)') i,atoms(i)%elem,atoms(i)%cx,atoms(i)%cy,atoms(i)%cz
         write(iu2,'(i4,x,a2,x,3f14.7)') i,atoms(i)%elem,atoms(i)%cx,atoms(i)%cy,atoms(i)%cz
      enddo
