...
module moparam
   ...
   use global, only: ne,iul,iull,MASTER,mytid,logmode
   use wfdata, only: nalpha,nbeta,nbas,norb,ndet,ncsf,aomocomb,useLAlib_mdet
   use mdet, only: cci,ndets,ccsf,deter,detia,detib,det1xa,det1xb,det1ya,det1yb,det1za,det1zb, &
                   det2a,det2b,mclist,lapack_inv,dgrad,dlapli,detsRepLst,detiaOne,detibOne
   use mos, only: cmo,mat,mat1x,mat1y,mat1z,mat2
   use utilsModule
   ...
   real*8, allocatable, target :: mok(:)
   real*8, allocatable, target :: mokgrad(:,:)
   real*8, allocatable, target :: moklapl(:)
   real*8, allocatable, target :: moklapli(:,:)
   real*8, allocatable         :: cmo0(:,:)
   real*8, allocatable         :: p_save(:)
...
contains
   ...
   real*8 tmp,tmp0,tmp1,tmp2,tmp3,tmp4,tmp5,coeff,d,darr(size(detsRepLst,1))
   real*8 ,allocatable ::  tmparr1(:,:),tmparr2(:,:),tmparr3(:,:),tmparr4(:,:)!keep results to avoid unnecessary calculations
   real*8, allocatable :: excitedMO(:), tmpdet(:,:), tmpdeti(:,:), tmpdetx(:,:),tmpdety(:,:),tmpdetz(:,:),tmpdet2(:,:)
   ...
   contains
      ...
      subroutine internal_ExcitOpAlphaSimple(newmo,pos,sign)
         ...
         if (.not. useLAlib_mdet) then
            call inv(tmpdet(1,1),na,na,tmpdeti(1,1),d)
         else
      ...
      subroutine internal_ExcitOpBetaSimple(newmo,pos,sign)
         ...
         if (.not. useLAlib_mdet) then
            call inv(tmpdet(1,1),nb,nb,tmpdeti(1,1),d)
         else
  ...
  subroutine moparam_calcderivsOnlyMok(ppMok,ie)
   ...
   real*8               :: coeff,darr(size(detsRepLst,1)),d
   real*8, allocatable  :: excitedMO(:), tmpdet(:,:), tmpdeti(:,:)
   integer, allocatable :: mclistexcit(:)
   real*8, intent(out)  :: ppMok (:)
...
do p=1,moParams
   ...
   !-------------------------------------!
      ...
      integer k,l,m,ierr,i,j,mu
      real*8 X(norb,norb), X2(norb,norb),W(norb,norb),R(norb,norb),W1(norb,norb)
      real*8 lambda(norb),tau(norb)
      real*8 zero,one
      ...
      if (mParamMode==SUCCESSIVE) then
         ...
         ! after testing replace by LAPACK
         call sdiag(X2,norb,norb,lambda,norb,W,norb,norb,norb,ierr)
         do i=1,norb
   ...
   subroutine rotateMO(i,j,kappa)
      ...
      real*8, intent(in)  :: kappa    ! rotation angle
      integer mu
      real*8 ci,cj
