module OptimizeParamsVarmin1Module
...
contains
   ...
   real*8, allocatable                  :: g(:),b(:),H(:,:)  ! gradient and Hessian
   real*8 emean, var, eOld, varOld, lambda,varRef,varRefOld,optERef,optLMlambda
   real*8                               :: eRef
    ...
    if(doWriteWF) call getPlusOptIter(optIter)
     ...
     call wfparams_set(WFP,p,.true.) ! normalization for ci coeffs
      if (doWritewf) then
         ...
         call abortp("varmin1_optimizeSample: write wf currently disabled!!!")
         !!!call writeWF(fname,.false.)
      end if
...
end module OptimizeParamsVarmin1Module
