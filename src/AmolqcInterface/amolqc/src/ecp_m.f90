module ecpModule
   ...
   !!use wfdata
   use global
   use jastrow, only: jasCalc, jasCalcInit, jasCalcUpdate, jasCalcInitWithUk, jasCalcUpdateWithUk
   use aomo, only: calc_aomo
   use mos, only: mo1calc
   use mdet, only: mdet2calc, resetToOld
   use utilsmodule, only: myran
   use Utils, only: double_findInSortedList
   use moparam, only:moparam_calcderivsOnlyMok,getMoUpdateMode
   ...
   integer, parameter, public :: TM_NONE=0, TM_SIMPLE=1, TM_SIZE_CONSISTENT=2
   ...
   type, public :: TMoveDataType
      ...
      real*8  :: tau = 0.d0              ! time step in T move
      real*8  :: ecpLocalW               ! local potential for weighting step
      real*8  :: ecpNonlocalW            ! non local potential for weighting step
...
!!!!!!!!!!!!!!!!!!!!!!!   ecp calculation   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   ...
   !--------------------------------------------------------------------------------------------
      ...
      real*8, intent(inout)            :: ecpLocal      ! return local ECP energy V_local
      real*8, intent(inout)            :: ecpNonlocal   ! and localized non-local contribution V_nonlocal (T move: at final coords)
      real*8, optional, intent(inout)  :: ecpNonlocalk(:) ! parameter derivative of V_nonlocal
      type(TMoveDataType), optional, intent(inout) :: TMoveData  ! if given, do T move, out: ecpNonlocal for Weighting

      real*8, allocatable              :: Vkk1(:,:)     ! V_k,k' discretised nonlocal potential (see Casula JCP 132, 154113)
      real*8, allocatable  ::  ints(:), vl(:)
      real*8  rrai, tmp, ecpLocalNew, ecpNonlocalNew, ecpLocalI, ecpNonlocalI
      ...
      do i = 1, size(Rdu%x)
         ...
         do pa = 1, size(this%pAtom)
            ...
            if (present(ecpNonlocalk)) then ! parameter derivatives
               ...
               block
                  real*8 :: intsvNlk(0:lmax-1,nParams)
               ...
               block
                  real*8 :: Vkk1perElec(ngp,0:lmax-1)
   ...
   !----------------------------------------------------------------------------------
      class(EcpType), intent(inout)    :: this
      type(RdataUpdate), intent(inout) :: Rdu           ! data structure for electron update calculations
      real*8, intent(in)               :: Vkk1(:,:)     ! V_k,k' discretised nonlocal potential (see Casula JCP 132, 154113)
      type(TMoveDataType), intent(inout)  :: TMoveData
      real*8 :: TNLpsum(size(Vkk1,1),size(Vkk1,2))        ! transition probabilities (partial sums)
      integer i, j, nElecs, nGridPoints, lb, ub, right, left
      real*8 xi, vecElecs(-1:size(Vkk1,2)), vecGridPoints(0:size(Vkk1,1))
      real*8 psum, noMoveProbability, ecpLocalTM, ecpNonlocalTM, norm
   ...
   subroutine ecp_sizeConsistentTMove(this, Rdu, Vkk1, i, TMoveData)
      class(EcpType), intent(inout)    :: this
      type(RdataUpdate), intent(inout) :: Rdu           ! data structure for electron update calculations
      real*8, intent(in)               :: Vkk1(:,:)     ! V_k,k' discretised nonlocal potential (see Casula JCP 132, 154113)
      ...
      real*8 psum, xi
      real*8 vecGridPoints(-1:size(Vkk1,1))
   ...
   subroutine ecp_paGridIntegration(this, Rdu, lmax, pa, ie, ints, intsvNlk, Vkk1perElec, idxVkk)
      ...
      real*8, intent(inout)            :: ints(0:)        ! integral values (for each l) by Lebedev quadrature
      real*8, intent(inout), optional  :: intsvNlk(0:,:)  ! integral values for parameter derivatives
      real*8, intent(inout), optional  :: Vkk1perElec(:,0:)  ! nonlocal contribution for electron ie at pa
      integer, intent(in), optional    :: idxVkk          ! start index in Vkk (first grid point)

      real*8 :: phi, theta               ! axis orientation (polar coords)
      real*8 :: rnew(3)                  ! new electron position
      real*8 :: vGrid(3),vGridRot(3),ww  ! grid points, rotated grid points and weights
      real*8 :: res
      real*8 :: rot(3,3)                 ! random rotation matrix
      real*8 :: costh, sinth, cosph, sinph
      real*8 :: costhn                   ! cos(theta')
      real*8 :: rai,raiVec(3)
   ...
   subroutine getVl(this, rai, pa, lmax, vl)
      type(EcpType), intent(in) :: this
      real*8, intent(in)        :: rai         ! elec-nuc distance
      integer, intent(in)       :: pa, lmax   ! current pseudo atom with lmax
      real*8, intent(inout)     :: vl(0:)      ! ecp coefficients V_l
