ccc aomocut_m.f contains the routines which are used if AO- and MO-cutoff are desired.
ccc In principle this routine could also be used for calculations without any cutoff
ccc or with AO-cutoff only but the routines in aomo_m.f are faster for that purpose.
ccc !!! WARNING !!!
ccc The ordering of the MO-coefficients in 'cmoa' is different depending on
ccc which routines are used. 'aomocut_calc' requires the cmoa array to be initialized by
ccc 'mocut' (in mos_m.f) !!!
ccc !!! WARNING !!!
...
c input parameters:
      ...
      real*8, dimension(nmax) :: x,y,z    ! x,y,z coordinates of position vector
      real*8, dimension(amax,nmax) :: rai ! r_ai electron-nucleus distances
