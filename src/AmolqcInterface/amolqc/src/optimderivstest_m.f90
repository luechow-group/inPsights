...
use utilsmodule, only: myran,init_ran
...
contains
   ...
   real*8                         :: tol     !tolorence for comparing num and analytical drivatives
   logical                        :: printRep
   ...
   type(RWSample), intent(inout) :: sample
   real*8, intent(in)            :: h            ! denominator for numerical differentiation
   integer, intent(in)           :: points       ! points for differentiation rule
   real*8, intent(in)            :: tol          ! tolorence for comparing num and analytical drivatives
   logical, intent(in)           :: printRep

   integer                       :: np,pnt,i,k
   real*8                        :: x(ne),y(ne),z(ne),u0, EL0, phi0
   real*8                        :: ELknum, Uk, phik
   real*8, allocatable           :: p(:),p0(:),f(:),g(:),uk0(:),ukgrad0(:,:),uklapl0(:)
   real*8, allocatable           :: uknum(:),ukgradnum(:,:),uklaplnum(:)
   real*8, allocatable           :: ELk(:),fi0(:)
