module OptimizeParamsWBFGSModule

   use global
   use RWSampleModule
   use ElocAndPsiTermsWEBFGSModule
   use WFParamsModule
   use lbfgsb3, only: setulb
   implicit none
...
contains
   ...
   real*8, allocatable                  :: g(:),g1(:)             ! gradient
   real*8, allocatable                  :: wk(:),wkEL(:),wELk(:)
   !!!real*8, allocatable                  :: s(:),y(:),bs(:),HB(:,:)
   integer optMode,maxiter,iflag
   real*8 e0,eps,eRef,var,stdDev
   real*8 w,wEL
...
end module OptimizeParamsWBFGSModule
