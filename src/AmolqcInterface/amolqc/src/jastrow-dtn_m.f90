module jastrowDTN
  ...
  real*8 :: cutOffEE, cutOffEN, cutOffEEN
  ! coefficients for power expansions
  logical :: allocatedParams = .false.
  real*8, allocatable :: alpha(:, :) ! 2, kmax
  real*8, allocatable :: beta(:, :, :) ! 2, amax, kmax
  real*8, allocatable :: gamma(:,:,:,:,:) ! 2, amax, 0:kmax, 0:kmax, 0:kmax
...
subroutine allocateParams()
  ...
  call assert(istat == 0, "jastrowDTN: parameter array allocation failed")
  alpha = 0
  beta = 0
  gamma = 0
...
subroutine jasdtnall(x, y, z, rai, rij, optType, ju, jud, julapl, julapli)
  ...
  real*8 :: uTerm, xTerm, fTerm
  ! nabla of correlation terms
  real*8 :: uDeriv(3*ne), xDeriv(3*ne), fDeriv(3*ne)
  ! laplacian
  real*8 :: uLapli(ne), xLapli(ne), fLapli(ne)
  ...
  real*8 :: minX, minY, minZ, maxX, maxY, maxZ, diffX, diffY, diffZ

  ! spin coefficient for e-e-term
  real*8 :: spinCoeff(ne, ne)
  ! numerator in e-n term, used to enforce cusp if necessary
  real*8 :: nucNum(nclast)
  ...
  ! powers of distances
  real*8 :: eePowers(ne, ne, -2:max(umax, feemax, 1))
  real*8 :: enPowers(nclast, ne, -2:max(xmax, fenmax, 1))

  ! derivatives of distances - 1st index: type (ai, aj, ij)
  real*8 :: rDeriv(3, 3), rLapl(3), rComb(3) !XXX maybe reuse in e-e-n?

  ! (distance - cut-off length) and powers
  real*8 :: dist, distj, distP(-1:C), distjP(-1:C)
  ! sum in x, u and f-term, derivative, nabla
  real*8 :: sumr, sumrd, sumrn, sumpr
  ! commonly used expressions in sum/derivative/nabla/laplacian of u/x/f
  real*8 :: uterms(5), xterms(5), fterms(8)
  real*8 :: fsum(8), fsuml(2)
  ! for parameter derivatives
  real*8 :: fsump(8), fsumpl(2)
  ...
  ! commonly used terms
  real*8 :: g, p, q, r, s, u, v, w, tmp, tmpa(3)
...
subroutine checkCusp()
  real*8 :: cuspMatrix(0:3 * fenmax + feemax + 1, fnum)
  real*8 :: paramVector(fnum)
  real*8 :: resVector(3 * fenmax + feemax + 2)
  real*8 :: ipiv(fnum)
  integer :: conditions
...
subroutine getConstraintsDTN(optMode, nConstraints, constraints, &
  ...
  real*8, intent(inout) :: constraints(:)
  real*8, intent(inout) :: constraintsGrad(:, :)
  real*8, intent(inout) :: constraintsHessian(:, :, :)
...
subroutine jasgeneric_naiv(x, y, z, rai, rij, optType, ju, jud, julapl, julapli)
  ...
  real*8 :: rijDeriv(ne, ne, 3), rijDerivj(ne, ne, 3), raiDeriv(nclast, ne, 3)
  ! (dr/dx)^2 + (dr/dy)^2 + (dr/dz)^2
  real*8 :: rijSquare(ne, ne), rijSquarej(ne, ne), raiSquare(nclast, ne)
  ! laplacian
  real*8 :: rijLapl(ne, ne), rijLaplj(ne, ne), raiLapl(nclast, ne)
  real*8 :: rComb(3)
  ...
  real*8 :: sumr, sumrd, sumrn, sumpr
  ! commonly used expressions in sum/derivative/nabla/laplacian of u/x/f
  real*8 :: uterms(4), xterms(4), fterms(12)
  real*8 :: fsum(8), fsuml(2)
  ! for parameter derivatives
  real*8 :: fsump(8), fsumpl(2)
  ! position of parameter for param derivatives
  ...
  ! commonly used terms
  real*8 :: g, p, q, r, s, u, v, w, tmp, tmp2, tmpa(3)
