...
implicit none
  ...
  real*8, allocatable  :: mWPlus(:),mWMinus(:)
  real*8, allocatable  :: mWSumPlus(:),mWSumMinus(:) 
...
contains
  ...
  !------------------------------!
    ...
    real*8 Wmean,wPSum,wMSum,wi,xi
  ...
  ! Our own implementation?! 
    ...
    real*8        :: wgtsum1,wgtsum2
    real*8        :: twgtsum1,twgtsum2
    real*8        :: tmp1,tmp2,xi
    real*8        :: tmp
    real*8        :: allWgt
    real*8        :: W
    real*8,allocatable        :: tmpWgt(:)
    ...
    do i = 1,count1
      xi  = myran()
      if (xi .le. recfW(i)%wn) then           
        count3 = count3 + 1 
    	  xi = myran()
        do j = 1,count2
          if (xi.le.recfW(j)%wp) then 	   
            count4 = count4 + 1
    ...
    if(count3.ne.count4) call abortp("qmc_reconf:No.of del. rwps .ue. copied rwps")
    call resettoMean(sample)
