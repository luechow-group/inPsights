module referenceModule
   ...
   use nextpsimaxModule, only: nextpsimax
   use Utils, only: tokenize,readFileLocal,readFileParallel,findInList
   use utilsmodule, only: getstra,getinta,getdbla
   use refBaseModule
   use refSimpleModule
   use refStrModule
   use refStrPermModule
   use refValModule
   use refValStrModule
   use refPosModule
   use refCtrModule
   use refDomainModule
   use refUtilsModule, only: calcElocRef, calcEigenRef
...
contains
   ...
   subroutine create_refContainer(rc_p,typ,listLength,doSortFreq,elemLength,valueThreshold,simThreshold,sameThreshold, &
      ...
      real*8, optional, intent(in)             :: valueThreshold
      real*8, optional, intent(in)             :: simThreshold
      real*8, optional, intent(in)             :: sameThreshold
      real*8, optional, intent(in)             :: simmaxThreshold
   ...
   subroutine references_check(lines,nl)
      ...
      real*8                         :: f,EL,vpot,meanDist,maxDist
      real*8                         :: fgrad(getNElec(),3)
      real*8                         :: nucThresh,coreThresh,bondThresh,hh,r(3)
      real*8, pointer                :: eval(:)=>null(),evec(:,:)=>null()
      integer                        :: a,i,j,iflag,iter,n,nrl,verb,iflag1,iflag2,iflag3
      ...
      if (MASTER) then
         ...
         if (writeEV) then
            ...
            do i=1,getNNuc()
               write(iu,'(i4,x,a2,x,3f14.7)') i,atoms(i)%elem,atoms(i)%cx,atoms(i)%cy,atoms(i)%cz
            enddo
         ...
         do n=1,refs%getSize()
            ...
            call calcEigenRef(rp,eval,evec,verb,hh,nucThresh)
            ...
            if (writeEV) then
               ...
               do i=1,size(eval)
                  ...
                  write(iu,'(i5)') size(eval)/3
                  do j=1,size(eval),3
                     write(iu,'(3f13.6)') evec(j:min(j+2,size(eval)),i)
                  end do
               end do
            end if
         end do
   ...
   subroutine references_check1(lines,nl)
      ...
      real*8                         :: ftol,tolSim,tolSame,tolDist2,hdist,f
   ...
   subroutine references_optimize(lines,nl)
      ...
      real*8                         :: hThresh,EL,vpot,f,thresh
      real*8                         :: eps(3)
      real*8                         :: fgrad(getNElec(),3)
      ...
      if (MASTER) then
         ...
         do n=1,refs%getSize()
            call refs%getElemPtr(n,p)
            rp => p%elem(1)
            call nextpsimax(rp%x,rp%y,rp%z,maxiter,hThresh,eps,verb,f,nCore,ierr)
            rp%count = 0
            rp%f = f
            if (verb >=1 .and. verb < 3) then
