...
module maxanalysisModule

   use global
   use referenceModule
   use ReferenceADT
   use atomModule, only: atoms_initPositionThresholds
   ...
   real*8              :: mFtol = 1.d-3    ! if the F(max) values of two maxima are <mFtol they are assumed equal
   real*8              :: mTolSim = 0      ! max distance for similar structures (bohr)
   real*8              :: mTolSame = 0     ! max distance for same structures (bohr)
   real*8              :: mTolDist2 = 0    ! max distance for "pos" structures (bohr)
...
contains
   ...
   !------------------------------!
      ...
      if (MASTER) then
         if  (maxMode=="val") then
            ...
            call abortp("$find_maxima: illegal value for max_mode=[val|vst|str|ctr|stp|pos]")
         end if
   ...
   real*8 function maxana_getFirstF()
      if (associated(mRC_p)) then
