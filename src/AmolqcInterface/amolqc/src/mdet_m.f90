...
use wfdata
use mos, only: mat,mat1x,mat1y,mat1z,mat2,mMOElecConfigs
...
implicit none
   ...
   real*8, allocatable  :: cci(:)      ! CI (multi CSF!) coeff.
   real*8, allocatable  :: ccsf(:,:)   ! CSF coefficients
   ...
   real*8, allocatable  :: deta(:,:,:),detb(:,:,:), det1xa(:,:,:),det1xb(:,:,:), &
                           det1ya(:,:,:),det1yb(:,:,:), det1za(:,:,:),det1zb(:,:,:), &
                           det2a(:,:,:), det2b(:,:,:)
   ! corresponding inverse matrices
   real*8, allocatable :: detia(:,:,:),detib(:,:,:), detiaOne(:,:,:),detibOne(:,:,:)
   real*8, allocatable :: det1xia(:,:),det1xib(:,:), &
   ...
   real*8, allocatable :: deter(:,:),dgrad(:,:,:),dlapli(:,:,:), olddet(:)
   ...
   real*8, allocatable :: T1xa(:,:), T1ya(:,:), T1za(:,:), T2a(:, :)
   real*8, allocatable :: T1xb(:,:), T1yb(:,:), T1zb(:,:), T2b(:, :)
   real*8, allocatable :: excma(:,:), excwa(:,:), excm1xa(:,:), excm1ya(:,:), &
...
! one determinant CSFs are generated here!
   ...
   if (nw >= 2) then
      ...
      do k=1,ncsf
         ...
         call assert(ndets(k)<=ndetcsfmax,'mdetcsfinput: ndet per csf too large (recompilation needed)')
         ndet = ndet + ndets(k)
         ! Give each determinant pair as orbital occupancy. First alpha
         ! electrons then beta electrons
         do j=1,ndets(k)
   ...
   if (logmode>=2  .and. MASTER) then
           write(iul,*)""
           write(iul,'(a,i5,a,i5)'),'    NCSFs = ', ncsf,' Ndets = ',ndet
...
! mdetoutput write CSFs to file unit 'iu'
   ...
   real*8, intent(inout) :: flapl(:),fgrad(:,:),flapli(:,:)
   integer, intent(in) :: w

   integer :: i, ii, j, jj, k, n, na, nci, orb, ierr
   real*8 :: d, tmp, f, g
   real*8 :: dxa(nalpha), dya(nalpha), dza(nalpha), d2a(nalpha)
   real*8 :: dxb(nbeta), dyb(nbeta), dzb(nbeta), d2b(nbeta)
   ...
   real*8 function fast_det(a,n,nmax,a1,det)
   !-----------------------------------!
      implicit none
      integer, intent(in) :: n,nmax
      real*8, intent(in) :: a(nmax,nmax), det
      real*8, intent(inout) :: a1(nmax,nmax)
      real*8 :: d
   ...
   !------------------------------------------------------!
   subroutine mdetcalc(ie,nec,phi,fgrad,flapli,flapl,iflag)
   !------------------------------------------------------!
...
! and faster evaluation of excited determinants.
   ...
   real*8, intent(inout)        :: phi(:)   ! Slater determinant part (for all elec configs)
   real*8, intent(inout)        :: flapl(:),fgrad(:,:),flapli(:,:)  ! derivatives of phi: laplacian and gradient
   integer, intent(out)         :: iflag    ! 0: OK, 1: LU decomp failed (sing matrix), 2: dgetri failed

   integer i,j,k,l,orb,ii,ii1,ii2,w
   integer nci,nnci,n,na,offset,ierr
   real*8 tmp,tmp1,tmp2,tmp3,tmp4
   real*8 d
   real*8, parameter :: RELSINGULAR = 1.d-12
   logical update
      ...
      if (nalpha==1) then
         ...
         if (ie == 0) then               ! all electrons new
            if (nci == 1) then
               ...
               if (.not. useLAlib_mdet) then
                  call inv(deta(1,1,nci),nalpha,nalpha,detia(1,1,nci),d)
               else
      ...
      if (nbeta==1) then
         ...
         if (ie == 0) then               ! all electrons new
            if (nci == 1) then
               ...
               ! returns inverse matrix in deti
               if (.not. useLAlib_mdet) then
                  call inv(detb(1,1,nci),nbeta,nbeta,detib(1,1,nci),d)
               else
...
! and faster evaluation of excited determinants.
   ...
   integer, intent(in)          :: ie       ! == 0 for all electron, > 0 for one electron calcn
   real*8, intent(inout)        :: phi      ! Slater determinant part
   ...
   real*8, intent(out)          :: phi   ! Slater determinant part
   integer, intent(in),optional :: ci_param_mode !if we need fk we need this
   real*8, intent(out),optional :: fk(:) ! divative are needed only for vNlk
   ...
   integer :: nci
   real*8 :: tmp
   real*8 :: olda(1:nalpha), oldb(1:nbeta)
   ...
   !---------------------------------------!
   subroutine lapack_inv(N,A,deti,det,iflag)
   !---------------------------------------!

   integer, parameter :: nb=64      ! Note:  nb=ilaenv(1,'dgetri','u',N,N,-1,-1)

   integer N                     ! real dimension of matrix A
   integer lwork                 ! work space dimensions
   real*8  det                   ! determinant of A
   real*8  p                     ! permutation P (see ipiv)
   real*8  A(:,:)                ! A and deti (A_LU) = P*L*U
   real*8  deti(:,:)             ! LU decomp. and inverse of A
   real*8  work(N*nb)            ! workspace!
   integer iflag                 ! on exit: 0: OK, 1: sing matrix, 2: error in dgetri
