module OptimizeParamsVarmin2Module
...
contains
   ...
   real*8, allocatable               :: x(:)       ! parameter vector
   real*8                            :: eOld, varOld, varRefOld,emean
   real*8                            :: var,varRef, optERef, eRef
  ...
  do
       ...
       go to 10 !<><><><><><><><> goto branch
         ...
         if (MASTER) then   ! NL2SOL masages
          if (logmode >= 2) then
            ...
            x = wfparams_get(WFP)
            call wfparams_set(WFP,x,.true.) ! normilize CI coeffs
            x = wfparams_get(WFP)
   ...
   real*8, intent(in)                   :: optERef
   real*8,intent(out)                   :: r(rSize)
   integer                              :: ierr
   real*8,allocatable                   :: elSend(:),elRec(:)
   ...
   ! caller is responsible for allocation of j array
    ...
    real*8,intent(inout)           :: j(rSize,nppp)
    real*8, allocatable            :: blockSend(:,:)
    real*8, allocatable            :: tblockSend(:,:)
    real*8, allocatable            :: blockRec(:,:)
    real*8, allocatable            :: tblockRec(:)
    integer                        :: ierr
   ...
   real*8,intent(inout)           :: blk(sSize,nppp)
   integer                        :: i,k,ii
   type(RandomWalker), pointer    :: rwp
   real*8                         :: x(ne),y(ne),z(ne),EL0
   real*8                         :: tmp,ELi(nppp)
   type(eConfigArray)  :: ec
   i=0
...
end module OptimizeParamsVarmin2Module
