...
module maxbasinsModule
   use global
   use wfdata, only: ncenter,atoms,nalpha
   use RandomwalkerModule
   use epartModule, only: epart_init, epart_add, epart_destroy, epart_write
   use utilsModule, only: assert, finda, getdbla, getinta, getstra
   use gcube_module
   use refVectorModule
   use statistics
   ...
   real*8                          :: mGridWidth=0.d0                           ! *half* the grid width
   ...
   real*8, allocatable             :: lrx(:,:),lry(:,:),lrz(:,:)
   ...
   real*8, allocatable             :: mTensor(:,:,:,:)
   real*8, allocatable             :: mEigenValues(:,:,:)
   real*8, allocatable             :: mLRmean(:,:,:)
   real*8, allocatable             :: mLRResult(:,:)                            ! 1) total count 2) ll/rr count, 3) lr/rl count
...
contains
   ...
   subroutine maxbas_init(lines,nl)
      ...
      real*8                          :: ax,bx,ay,by,az,bz
      real*8                          :: cx(ncenter),cy(ncenter),cz(ncenter)
      real*8                          :: tolSim
      ...
      if (iflag /= 0) call abortp("$init_max_domain: kmax required")
      ...
      if (iflag1 /= 0) call abortp("$init_basin_analysis: ref_file argument required")
      allocate(mRV_p)
      if (iflag2 == 0) then
      ...
      if (iflag3 /= 0 .and. mDoLRAnalysis) call abortp("$init_basin_analysis: lralist argument required")
   ...
   contains
      ...
      subroutine internal_cube_init()
         ...
         if (iflag /= 0) then
            ...
            if (iflag /= 0) call abortp("(maxbas_init): either grid or ax,bx,.. required for max domain calculation")
            ...
            if (ax > bx .or. ay > by .or. az > bz) call abortp('(maxbas_init): cube definition requires a < b')
         else
      ...
      subroutine internal_xyz_init()
         ...
         do i=1,ncenter
             write(mIU,'(i4,x,a2,x,3f14.7)') i,atoms(i)%elem,atoms(i)%cx,atoms(i)%cy,atoms(i)%cz
         enddo
   ...
   subroutine maxbas_add(x,y,z,f,rw,idx1,iflag)
      ...
      if (((mDoEPart .or. mAnalyseEPairs==2) .and. .not.isnan(f)) .and. iflag==0) then
   ...
   subroutine maxbas_addSerially(x,y,z,x0,y0,z0,f,f0,idx1,iflag)
      ...
      real*8,intent(in)   :: x(:),y(:),z(:)      ! max coords
      real*8,intent(in)   :: x0(:),y0(:),z0(:)   ! original coords
      real*8, intent(in)  :: f,f0                ! function values (-ln(psi^2))
      integer, intent(in) :: idx1(:), iflag
      integer             :: allidx1(nproc*ne)
      real*8              :: vec(6*ne+2)
      real*8              :: vec_rcv((6*ne+2)*nproc)
      integer             :: ierr,i,n,m,l,k,j
      integer             :: idx2(ne),idx(ne),int_rcv(nproc)
      type(reference)     :: r
      real*8, parameter   :: EPS = 1.d-4
      real*8              :: temp1(3),temp2(3),tempMat(2,2)
         ...
         do i=0,nproc-1
            ...
            if (int_rcv(i+1) == 0) then
               ...
               if (m > 0) then
                  ...
                  ! write idx of permutation
                  if (mWriteIdx) call maxbas_writeIdx(m,idx2)
   ...
   contains
      ...
      subroutine internal_addCube(m,x,y,z,idx,val)
         integer    :: m                ! maximum
         real*8     :: x(:),y(:),z(:)   ! electron coords
         integer    :: idx(:)           ! permutation
         real*8     :: val              ! walker weight
         integer i,ii
      ...
      subroutine internal_printDistToPermRef()
         ...
         integer j,jj
         real*8 xx,yy,zz,xx1,yy1,zz1,d,dsum,d1sum
   ...
   subroutine maxbas_writeResults()
      ...
      integer*8                                 :: nd,mynd
      real*8                                    :: pairRatio,stddev,mymean,mystddev
      real*8                                    :: tempMat(3,2,2)
      real*8                                    :: tempMat0(2,2)
      real*8, parameter                         :: PAIRTHRESH = 0.95d0, WEAKPAIR=0.1d0, &
      ...
      if (mAnalyseEPairs > 0) then
         ...
         do m=1,mKMax
            ...
            np = 0
            do i=mNCore+1,nalpha
               do j=i+1,nalpha
                  np = np + 1
                  call internal_writePair()
               enddo
               do j=nalpha+mNCore+1,ne
                  np = np + 1
                  call internal_writePair()
               enddo
            enddo
            do i=nalpha+mNCore+1,ne-1
               do j=i+1,ne
                  np = np + 1
                  call internal_writePair()
               enddo
   ...
   subroutine maxbas_writeIdx(m,idx)
       integer, intent(in)    :: m                ! maximum
       integer, intent(in)    :: idx(:)           ! permutation

       if (mWriteIdx) then
           if (.not.mWriteXYZ) mLCount = mLCount + 1
           write(mIU1,'(i9,i5)') mLCount,m
           write(mIU1,'(50i4)') idx
       endif
   end subroutine maxbas_writeIdx
   ...
   subroutine maxbas_collectLR(x,y,z,m,pitensor,pev)
      real*8, intent(in)                         :: x(:),y(:),z(:)
      integer, intent(in)                        :: m                ! maximum
      real*8, allocatable                        :: itensor(:,:,:)
      real*8, intent(out)                        :: pitensor(:,:,:)
      real*8, allocatable                        :: ev(:,:)
      real*8, intent(out)                        :: pev(:,:)
      real*8                                     :: work(8),diff
      real*8                                     :: tempMat(3,3)
      real*8                                     :: tempVec(3)
