...
module RdataUpdateModule
   ...
   use global
   use wfdata, only: calcNucElecDists, calcElecElecDists, atoms, ncenter
   implicit none

   type, public :: RdataUpdate
      ! position data
      real*8, pointer :: x(:) => null()      ! all electrons cartesian coordinates
      real*8, pointer :: y(:) => null()      ! all electrons cartesian coordinates
      real*8, pointer :: z(:) => null()      ! all electrons cartesian coordinates
      real*8, pointer :: rai(:,:) => null()  ! elec-nuc distance matrix
      real*8, pointer :: rij(:,:) => null()  ! elec-elec distance matrix
      real*8  :: xold=0, yold=0, zold=0        ! save old electron coordinates
      real*8, pointer :: raiOld(:), rijOld(:)  ! save old electron distances
      real*8, pointer :: raibarOld(:,:) => null()  ! save old \bar{r_{ai}}^v
      ! result data
      real*8 :: phi=0.d0, U=0.d0             ! results: phi, U
      real*8 :: phi0=0.d0, U0=0.d0           ! results: initial electron position: phi, U
      real*8, pointer :: phik(:) => null()   ! parameter derivative \partial_k\Phi
      real*8, pointer :: Uk(:) => null()     ! parameter derivative \partial_k U
      real*8, pointer :: Uk0(:) => null()    ! initial parameter derivative \partial_k U
      real*8, pointer :: mok(:) => null()    ! parameter derivative w.r.t. orbital rotation k
      ! intermediate data for jastrow updates
      real*8, pointer :: Fij(:,:) => null()  ! jastrow one electron update terms (ee+een)
                                             ! storage: Fij(j,i) with i<j. Note order!
      real*8, pointer :: Gi(:) => null()     ! jastrow one electron terms (en+aos)
      real*8, pointer :: Fijold(:) => null() ! save original values
      real*8, pointer :: Fijk(:,:,:) => null()  ! Fij for each parameter deriv k
      real*8, pointer :: Fijkold(:,:) => null() ! save original values
      real*8, pointer :: Gki(:,:) => null()     ! Gi for each parameter deriv k
      real*8, pointer :: Gkiold(:) => null()    ! save originial values
      real*8  :: Giold=0, Xiold=0
      ! misc
...
contains
   ...
   subroutine RdataUpdate_init(this,x,y,z,rai,rij)
      ...
      real*8, intent(in), optional  :: rai(:,:), rij(:,:)
