module refBaseModule
   ...
   use ReferenceADT
   use RefVListModule
   use ReflistVListModule
   use refUtilsModule, only: calcRefDifference
   ...
   type, abstract :: referenceContainer
      ...
      character(len=120) :: infoMessage = "no initial references provided"
...
contains
   ...
   subroutine refc_setInfoMessage(this,s)
      ...
      if (this%infoMessage == "no initial references provided") then
         ...
         write(this%infoMessage,'(3A)') trim(this%infoMessage),CHAR(13)//CHAR(10),trim(s)
      endif
   end subroutine refc_setInfoMessage


   real*8 function refc_getFirstF(this)
      class(referenceContainer), intent(inout) :: this
      type(reference), pointer                 :: rp
      type(refl_vlist), pointer                :: p
