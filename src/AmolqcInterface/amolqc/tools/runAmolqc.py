...
class Amolqc:
   ...
   def __init__(self,mol,orbs,cfg='amolqc.cfg',verbose=True):
      ...
      self.defqueue = ifile.readline().rstrip('\n')
      self.defnproc = ifile.readline().rstrip('\n')
      self.defjobname = ifile.readline().rstrip('\n')
      self.jobTemplate = ifile.readlines()
      self.jobFilename = 'job'
   ...
   def write_job(self,queue="",nproc="",jobname=""):
      ...
      for line in self.jobTemplate:
         if "QUEUE" in line:
            ...
            line = line.replace("NPROC",str(nproc))
   ...
   def copy_ref(self,suffix,dest):
      """ copy ref file to Jastrow """
      if suffix == "":
      	 refname = self.name+".ref"
      else:
      	 refname = self.name+"-"+suffix+".ref"
      src = self.maxdir+"/"+refname
      if dest=="SED" or dest=="s":
         dst = self.seddir
      else:
         raise AmolqcMaster("unknown dest")
      if self.job:
   ...
   def create_vm(self,suffix,jastrow,E_ref,seed=101,size=500,wf="",sblocks=40,vblocks=20,cnt=5,iter=3,stddev=0.001):
      ...
      inf.write("$generate_sample(size="+str(size)+",blocks="+str(sblocks)+")\n")
      inf.write("$jastrow_varmin1(E_ref="+str(E_ref)+",blocks="+str(vblocks)+",max_iter="+str(iter)+",count="+str(cnt)+")\n")
   ...
   def create_em(self,suffix,eqblocks,seed=101,size=500,cnt=5,wf="",testwf=True):
      ...
      inf.write("$jastrow_emin(count="+str(cnt)+",blocks="+str(eqblocks)+")\n")
      inf.write("$wf(write,file='"+wfname+"')\n")
      inf.close()
   ...
   def create_max(self,suffix,seed=101,size=50,blocks=200,itmax=150,nmax=100,fmax=20,tol=0.1,pos=0.02,hdist=0.1,wf=""):
      ...
      inf.write("$wf(read,file='"+wf0name+"')\n")
      inf.write("$generate_sample(size="+str(size)+")\n")
      inf.write("$find_maxima(blocks="+str(blocks)+",steps=200,max_iter="+str(itmax)+",max_mode=str,\n")
      inf.write("   nmax="+str(nmax)+",max_full="+str(fmax)+",tol_maxdist="+str(tol)+",tol_pos="+str(pos)+",H_dist="+str(hdist)+")\n")
      inf.close()
   ...
   def create_seds(self,suffix,max="",ref=1,seed=101,size=200,blocks=200,stddev=0.0002,grid=4.0,wf=""):
      ...
      inf.write("$generate_sample(size="+str(size)+")\n")
      inf.write("$sed(vmc,move=umr,steps=200,blocks="+str(blocks)+",discard=5,accept_ratio=0.5,stddev="+str(stddev)+",\n")
      if max=="":
         inf.write("     epart,readref,ref="+str(ref)+",grid="+str(grid)+")\n")
      else:
         maxfile = self.name+"-"+max+".ref"
         inf.write("     epart,ref_file="+maxfile+",ref="+str(ref)+",grid="+str(grid)+")\n")
      inf.close()
