...
#                 vars[a]['dimension'].reverse()
            vardef='%s, dimension(%s)'%(vardef,','.join(vars[a]['dimension']))
        if 'intent' in vars[a]:
            lst = true_intent_list(vars[a])
            if lst:
                vardef='%s, intent(%s)'%(vardef,','.join(lst))
        if 'check' in vars[a]:
            vardef='%s, check(%s)'%(vardef,','.join(vars[a]['check']))
        if 'depend' in vars[a]:
            vardef='%s, depend(%s)'%(vardef,','.join(vars[a]['depend']))
        if '=' in vars[a]:
            v = vars[a]['=']
