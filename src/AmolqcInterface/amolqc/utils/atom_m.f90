...
module atomModule
   ...
   type atom
      real*8            :: cx = 0   ! x,y,z coordinates (in bohr)
      real*8            :: cy = 0
      real*8            :: cz = 0
      real*8            :: za = 0
      integer           :: elemIdx = 0
   ...
   integer :: mNAtoms = 0
   real*8, allocatable :: mAtomDist(:,:)

   ! thresholds for atoms_whatPosition
   real*8  :: mNucThresh = 0.01
   real*8  :: mCoreThresh = 2.0
   real*8  :: mBondDiff = 0.4
...
contains
   ...
   if (present(atomIdx)) then
      ...
      call abortp('atom: either element number or symbol required')
   end if
   ...
   integer, intent(in)           :: iu
   integer :: i
   real*8  :: x(size(self)),y(size(self)),z(size(self))
   ...
   !--------------------------------------------------!
      ...
      real*8, intent(inout)  :: ax,bx,ay,by,az,bz
   ...
   !---------------------------------------------------------------------!
      real*8, intent(in)  :: nucThresh,coreThresh,bondDiff
   ...
   !---------------------------------------------------!
      ...
      real*8                  :: xnew(size(x)),ynew(size(x)),znew(size(x))
      real*8                  :: omega(size(self))
      real*8                  :: rai(size(self),size(x))
      integer i,a
      real*8 sum
      real*8, parameter       :: RMIN = 1.d-8
...
! internal functions
   ...
   if (i>PSEMAX) call abortp('getPSEIdx: Element not yet known')
   getPSEIdx = i
